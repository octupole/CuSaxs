...
def form_factor_with_background(q, delta, delta_rho, background):
    ...
    return (
        ...
        * (delta_rho**2)
        / (q**2)
        * (1 - np.cos(q * delta) * np.exp(-(q**2) * sigma**2 / 2))
        + background
    )
...
def fit_data(q, P):
    initial_guess = [
        ...
        100.0,
        10000.0,
    ]  # initial guess for [delta, delta_rho, background]
    ...
    popt, pcov = curve_fit(
        form_factor_with_background, q, P, p0=initial_guess, bounds=bounds
    )
    return popt, pcov
...
def plot_results(q, P, popt):
    ...
    plt.plot(
        q_fit, form_factor_with_background(q_fit, *popt), "r-", label="Fitted curve"
    )
    plt.xlabel("q (Å⁻¹)")
    plt.ylabel("P(q)")
    plt.legend()
    plt.title(
        f"Form Factor Fitting (δ = {popt[0]:.2f} Å, ΔF = {popt[1]:.2f}, bg = {popt[2]:.4f})"
    )
    plt.show()
...
def main():
    parser = argparse.ArgumentParser(
        description="Fit form factor data with background and constrained bilayer thickness."
    )
    parser.add_argument(
        "-f", "--file", required=True, help="Input file with q and P(q) data"
    )
    parser.add_argument(
        "--plot", action="store_true", help="Plot input data and fitted curve"
    )
