...
def main(
    ...
    cons = {
        "type": "ineq",
        "fun": lambda alpha: constraint(
            alpha, q_regular, I_exp_interp, I_sol_interp, q_min_fit, q_max_fit
        ),
    }
    result = minimize(
        fit_function,
        x0=1.0,
        args=(q_regular, I_exp_interp, I_sol_interp, q_min_fit, q_max_fit),
        method="SLSQP",
        constraints=cons,
    )
    ...
    min_difference = np.min(
        difference[(q_regular > q_min_fit) & (q_regular <= q_max_fit)]
    )
    if min_difference < 10.0:
        warnings.warn(
            f"Constraint not fully satisfied. Minimum difference in fitting range: {min_difference:.2f}"
        )
    ...
    np.savetxt(
        output,
        np.column_stack((q_regular[mask_diff], difference[mask_diff])),
        fmt="%.6f",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fit and compare two functions with constraints, allowing negative alpha."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Number of points in the regular grid",
    )
    parser.add_argument(
        "--q_min_fit", type=float, default=1.7, help="Minimum q value for fitting range"
    )
    parser.add_argument(
        "--q_max_fit", type=float, default=2.5, help="Maximum q value for fitting range"
    )
    parser.add_argument(
        "--q_max_diff",
        type=float,
        default=2.0,
        help="Maximum q value for difference calculation",
    )
